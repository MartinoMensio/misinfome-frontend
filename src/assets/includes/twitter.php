<?php
	$twtrfile = $_SERVER['DOCUMENT_ROOT'] ."/quali-data/themes/qualichain/assets/includes/qc-tweets.txt";
	...
	foreach($twtr->statuses as $itemdata):
		//if(!isset($itemdata->retweeted_status)):
			$t++;
			echo '<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12"><article class="twitter">';
			if(!empty($itemdata->entities->media[0]->media_url)):
				$media = '<span class="tweet-media"><p><a href="https://'.linkify_tweet($itemdata->entities->media[0]->display_url).'" target="_blank"><img src="'.preg_replace("/^http:/i", "https:", $itemdata->entities->media[0]->media_url).'" alt="twitter media" /></a></p></span>';
			else:
				$media = '';
			endif;
			echo '<div class="twitter-article" id="'.$t.'"><div class="item">';
			echo '<div class="twitter-pic"><a href="https://twitter.com/'.$itemdata->user->screen_name.'" target="_blank"><img src="'.preg_replace("/^http:/i", "https:", $itemdata->user->profile_image_url).'" width="48" height="48" alt="twitter icon" /></a></div>';
